  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        disabled={!ready}
        className="thumb-line h-11 w-auto gap-1.5 px-4 t-small disabled:border-line disabled:text-faint"
      >
...
export function Memory({
  ...
  return (
    <section>
      ...
      <div className={cn("mt-4 px-5", now && now.id !== first.id ? "grid grid-cols-2 gap-2" : "")}>
        <Beat post={first} kicker="primeiro" onOpen={onOpen} />
        {now && now.id !== first.id ? <Beat post={now} kicker="agora" onOpen={onOpen} /> : null}
      </div>
      {path.length > 2 ? (
        <ul className="mt-5 space-y-4 px-5">
          {path.slice(1, -1).slice(0, 4).map((p) => (
            <li key={p.id}>
              <button type="button" onClick={() => onOpen(p.id)} className="press w-full text-left">
                <Shot src={p.image} video={p.video} still square />
                <p className="t-kicker mt-2">no caminho</p>
                <p className="t-body mt-1">{markOf(p) || p.caption || "Sessão"}</p>
                <p className="t-small mt-0.5">{dateShort(dayOf(p.hoursAgo))}</p>
              </button>
            </li>
          ))}
        </ul>
