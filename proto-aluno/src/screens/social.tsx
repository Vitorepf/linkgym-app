...
import { ActionBar, Card, Chip, MarkStrip, ObjectBar, Reveal, Roll, Row, Scroll, SectionHead } from "@/ui/kit";
...
export function Prova() {
  ...
  const duel = proof
    ...
    : undefined;
  const faixa = duel?.veredito?.faixa[YOU_ID];
  const [showMark, setShowMark] = useState(Boolean(faixa));
  useEffect(() => {
    if (!faixa) {
      setShowMark(false);
      return;
    ...
    setShowMark(true);
    const t = window.setTimeout(() => setShowMark(false), 520);
  ...
  const digit =
    own && proof.kg != null
      ? String(proof.kg)
      : (faixa?.match(/(\d+(?:[.,]\d+)?)/)?.[1] ?? (proof.kg != null ? String(proof.kg) : "—"));
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {showMark && faixa ? (
        <div data-dock className="px-5 pt-3 pb-3">
          <MarkStrip mark={digit} line={faixa} />
        </div>
      ) : (
...
export function Pessoa() {
  ...
  const spot = arenaSpot(person.id);
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <Scroll>
        ...
        >
          <div className="mt-5">
            <Duo
              a={{ k: "arena", v: `${spot.pos}º`, unit: `de ${spot.of.toLocaleString("pt-BR")}` }}
              b={{ k: "semana", v: String(week), unit: "dias" }}
            />
          </div>
          <div className="mt-4">
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <Scroll>
        ...
        {kind === "texto" ? null : (
          <div className="mt-5 px-5">
            <SectionHead>Escolha uma</SectionHead>
            <ul className="mt-3 space-y-4">
              {PICKS.slice(0, 4).map((p) => {
                const on = image === p.src;
                return (
                  <li key={p.src}>
                    <button
                      type="button"
                      className="press h-12 w-full text-left"
                      aria-pressed={on}
                      onClick={() => setImage(p.src)}
                    >
                      <img src={p.src} alt="" className="aspect-square w-full object-cover" />
                      <p className="t-kicker mt-2">{on ? "escolhida" : "foto"}</p>
                      <p className="t-body mt-0.5">{p.label}</p>
                    </button>
                  </li>
                );
              })}
      ...
      <ActionBar>
        <Thumb
          ...
          onPress={() => publish(kind, caption, image)}
        />
      </ActionBar>
    </div>
  );
