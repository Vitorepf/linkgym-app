      ...
      {TABS.map((t) => {
        ...
        return (
          ...
          >
            ...
            <span className={cn("t-micro", on ? "font-semibold" : "font-medium")}>{t.label}</span>
          </button>
        );
      })}
    </nav>
...
/** Rodapé de ação do rack. Uma primária, no máximo uma saída silenciosa. */
export function Dock({ children }: { children: React.ReactNode }) {
  return (
    <div
      data-dock
      className="shrink-0 border-t border-line bg-dock px-5 pt-3 pb-[max(14px,env(safe-area-inset-bottom))]"
    >
