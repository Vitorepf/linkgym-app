  ...
  const body = (
    <>
      ...
      <div className="mt-3 flex items-end justify-between gap-4">
        ...
        <p className="t-score">{homeScore}</p>
      </div>
...
export function Display({ children, className }: { children: ReactNode; className?: string }) {
  return <h1 className={cn("t-display text-ink", className)}>{children}</h1>;
}
...
export function HoldTick({ onTick, label }: { onTick: () => void; label: string }) {
  ...
  return (
    <button
      ...
      className="flex h-14 flex-1 items-center justify-center bg-fill text-ink t-body active:bg-raised"
    >
      {label}
    </button>
  );
