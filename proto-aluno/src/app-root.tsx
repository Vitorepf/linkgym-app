...
/** O rack é uma corrente: avança de lado. Sobreposição sobe. Aba atravessa. */
const RACK = new Set<string>(["serie", "descanso", "feito", "como", "fichaSessao"]);
...
export function AppRoot() {
  ...
  useEffect(() => {
    ...
    void Promise.resolve(useLink.persist.rehydrate()).then(() => {
      ...
      if (s.session && !s.cumprido) {
        const keep = s.overlay != null && RACK.has(s.overlay);
        if (!keep) {
  ...
  const isRack = overlay != null && RACK.has(overlay);
  const isSheet = overlay != null && !RACK.has(overlay);
    ...
    <PhoneShell>
      <div className="relative flex min-h-0 flex-1 flex-col">
        {isRack && OverlayScene ? (
          ...
          >
            <SceneBoundary key={overlay} onLeave={goHome}>
          ...
          <motion.div
            ...
            className="flex min-h-0 flex-1 flex-col"
          >
            <SceneBoundary key={tabKey} onLeave={goHome}>
              <Suspense fallback={<div className="flex-1" />}>
                <TabScene />
              </Suspense>
            </SceneBoundary>
        ...
        {isSheet && OverlayScene ? (
          <Sheet onDismiss={dismissSheet}>
            <motion.div
              key={overlay}
              initial={from}
              animate={{ opacity: 1, y: 0 }}
